...
model <- function(t, state, parms) {
  with(as.list(c(state,parms)), {
    ...
    R <- s/(d+a1*N1+a2*N2)
    dN1 <- c1*((a1*R)/(k1+R))*N1-d1*N1
    dN2 <- c2*((a2*R)/(k2+R))*N2-d2*N2

    return(list(c(dN1,dN2)))  
  }) 
}

p <- c(s=5,d=0.5,a1=0.4,a2=0.6,c1=0.24,c2=0.2,d1=0.09,d2=0.1,k1=0.1,k2=0.3)

s <- c(N1=1,N2=1)

run(tmax=1000, tstep=0.1,xlab="Time [years]",ylab="Population size")
