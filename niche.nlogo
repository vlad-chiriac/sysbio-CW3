...
to go
  ...
  grow-grass
  ask turtles [ ; go through all fishes concurrently. don't first go though all fishA, then fishB, because then the early bird (fishA) gets all the worms (seagrass)
    (ifelse
      is-fishA? self [
        move_fishesA
        eat-grass
...
;; grow grass randomly. One could consider a more logistic growth approach, in which seagrass gives rise to more seagrass
to grow-grass
  ask patches [
    if pcolor = black [
      if random-float 1000 < grass-growth-rate
        [ set pcolor green ]
  ] ]
end
...
to reproduce_fishesA
  ...
  if energy > fishesA_birth_threshold
    [ set energy energy / 2
      hatch 3 [ fd 1 ] ] ; three fish get born and move a little bit away
end

to reproduce_fishesB
  ;; give birth to new fish, which takes half of energy
  if energy > fishesB_birth-threshold
    [ set energy energy / 2
      hatch 1 [ fd 1 ] ] ; one fish gets born and move a little bit away
end
