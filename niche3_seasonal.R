...
model <- function(t, state, parms) {
  with(as.list(c(state,parms)), {
    ...
    R <- s/(d+a1*N1+a2*N2)
    dN1 = ifelse((t%%12)>6 , c1*a1*R*N1-d1*N1 , -d1*N1)
    dN2 = ifelse((t%%12)<7 , c2*a2*R*N2-d2*N2 , -d2*N2)
    
    return(list(c(dN1,dN2)))  
  }) 
}

#N1 live fast die fast, N2 live long die slow
#a is how much they eat, c is efficiency of converting that food into bebes
#dN is death, dR is resource turnover
p <- c(s=30,d=0.5,a1=0.3,a2=0.5,c1=0.3,c2=0.06,d1=0.3,d2=0.1)

s <- c(N1=6,N2=6)

run(method="ode45",tmax=1200, tstep=12,xlab="Time [months]",ylab="Population size")

end <- run(method="radau",tmax=12000, tstep=12,xlab="Time [months]",ylab="Population size")
run(method="radau",state=end,tmax=120000, tstep=1,xlab="Time [months]",ylab="Population size")
