...
model <- function(t, state, parms) {
  with(as.list(c(state,parms)), {
    ...
    R <- (s/(d+a1*N1+a2*N2))
    dN1 <- c1*a1*(R*(1+(1/N1)))*N1-d1*N1
    dN2 <- c2*a2*(R*(1+(1/N2)))*N2-d2*N2

    return(list(c(dN1,dN2)))  
  }) 
}

p <- c(s=5,d=0.5,a1=0.4,a2=0.6,c1=0.24,c2=0.2,d1=0.09,d2=0.1)

s <- c(N1=1,N2=1)

run(tmax=1000, tstep=1,xlab="Time [years]",ylab="Population size")
end <- run(tmax=10000, tstep=0.1,xlab="Time [years]",ylab="Population size")
run(state = end,tmax=10, tstep=0.1,xlab="Time [years]",ylab="Population size")
...
plane(xmin=-0.001,ymin=-0.001,xmax=10,ymax=15,eps=-.001,show=names(s),zero=F,tstep=0.5,vector=T,portrait=T) 
