...
to grow-seaweed
  ask patches [
    if pcolor = black [
      if random-float 1000 < seaweed-growth-rate
        [ set pcolor green ]
  ] ]
end
...
to reproduce_fishesA
  ...
  if energy > fishesA_birth_threshold and ticks mod 364 <= fishesA_mating_days
    [ set energy energy / 2
...
to reproduce_fishesB
  ...
  if energy > fishesB_birth_threshold and ticks mod 364 <= fishesB_mating_days
    [ set energy energy / 2
